...
import { defaultForm } from "./caseForm.defaults";
...
// defaultForm imported from ./caseForm.defaults
...
const CaseForm = ({ formData, setFormData, onCancel, onSubmit, isEdit }) => {
  ...
  const fetchDropdownData = async () => {
    try {
      const [courtRes, locationRes, judgeRes, subjectRes, statusRes] =
        await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedJudges.length === 0) {
      notify.error("Please select at least one judge");
      return;
    }
    ...
    try {
      ...
      if (onCancel) {
        ...
        // Navigate to cases list so the user sees the new case
        navigate("/cases");
      }
    } catch (error) {
      console.error("Submit failed:", error);
      if (error.response && error.response.data && error.response.data.error) {
        notify.error("Failed to submit case: " + error.response.data.error);
      } else {
        notify.error("Failed to submit case.");
      }
    }
  };
