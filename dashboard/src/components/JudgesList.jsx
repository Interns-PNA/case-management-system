...
const JudgesList = () => {
 ...
 const handleSearch = (term) => {
  ...
  const handleUpdateJudge = async (updatedJudge) => {
    try {
      await axios.put(`http://localhost:5000/api/judges/${updatedJudge._id}`, updatedJudge);
      fetchJudges();
      setEditJudge(null);
    } catch (err) {
  ...
  return (
    <div className="courts-list">
      ...
      {editJudge && (
        <EditJudgeModal judge={editJudge} onClose={() => setEditJudge(null)} onUpdate={handleUpdateJudge} />
      )}
    </div>
  );
};
