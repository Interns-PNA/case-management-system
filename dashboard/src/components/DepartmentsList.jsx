...
const DepartmentsList = () => {
  ...
  const handleSearch = (term) => {
    ...
    const filtered = departments.filter((dept) =>
      ...
      (dept.details && dept.details.toLowerCase().includes(term.toLowerCase()))
    );
  ...
  const handleUpdateDepartment = async (updatedDept) => {
    try {
      await axios.put(`http://localhost:5000/api/departments/${updatedDept._id}`, updatedDept);
      fetchDepartments();
      setEditDept(null);
    } catch (err) {
  ...
  {filteredDepartments.map((dept, index) => (
    <div className="departments-table-row" key={dept._id}>
      ...
      <span className="actions">
        <button className="btn-edit" onClick={() => setEditDept(dept)}>Edit</button>
        <button className="btn-delete" onClick={() => handleDeleteDepartment(dept._id)}>Delete</button>
      </span>
    </div>
  ))}
</div>
